...
File:
scripts/python/train_random_forest_baseline.py
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Train Random Forest baseline for BggDeepLearning."
    )
...
def main() -> None:
    ...
    from bggdeep.models.traditional.random_forest import (
        RandomForestBaselineConfig,
        RandomForestBaselineTrainer,
        build_confusion_matrix_table,
        build_model_comparison_table,
        load_processed_tabular_data,
        save_random_forest_outputs,
    )
    ...
    logger.info("Random Forest baseline training started")
    ...
    model_config = RandomForestBaselineConfig(
    ...
    trainer = RandomForestBaselineTrainer(model_config)
    ...
    logger.info("Number of trees: %s", model_config.n_estimators)
    ...
    model_comparison_df = build_model_comparison_table(
        random_forest_metrics=metrics_df,
        table_dir=project_paths.table_dir,
    )

    output_paths = save_random_forest_outputs(
        trainer=trainer,
    ...
    logger.info("Random Forest baseline training finished")
    ...
    print("=" * 70)
    print("BggDeepLearning Random Forest baseline training succeeded")
    print("-" * 70)
